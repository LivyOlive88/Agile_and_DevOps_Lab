...
- **Environment Differences:** I encountered a tricky issue where PowerShell's `echo` command created a UTF-16 encoded `requirements.txt`, which broke `pip install` on the Linux CI runner. This taught me to be careful with cross-platform text handling.
...
## 3. Improvements for Future Sprints
- **Interactive Dashboards:** Instead of static PNGs, I could use Plotly or Streamlit for interactive data exploration.
